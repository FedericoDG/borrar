...
app.get('/profesionales', (req, res) => {
  const consulta =
    'SELECT * FROM profesional where especialidad <> "Administrador" order by especialidad';
...
app.get('/agenda', (req, res) => {
  ...
  if (!fecha) {
    ...
    const month = String(today.getMonth() + 1).padStart(2, '0'); // Los meses comienzan en 0 const day = String(today.getDate()).padStart(2, '0');
...
app.get('/historia/:dniPaciente/:idAgenda/:motivoConsulta', async (req, res) => {
  ...
  const medicamentos = await new Promise((res, rej) => {
    // trae los medicamentos del paciente
  ...
  const paciente = await new Promise((res, rej) => {
    // datos del paciente
    const consulta = `SELECT DISTINCT p.nombre, p.dni, p.nacimiento, p.sexo, p.telefono
     FROM paciente p     
    WHERE p.dni = ? `;
  ...
  const medico = await new Promise((res, rej) => {
    // datos del paciente
  ...
  const alergias = await new Promise((res, rej) => {
    // trae las alergias del paciente
  ...
  const antecedentes = await new Promise((res, rej) => {
    // trae los antecedentes del paciente
    const consulta = `SELECT  *
        FROM antecedentes a  
        WHERE a.dniPaciente = ? `;
    connection.query(consulta, [dniPaciente], function (err, results) {
  ...
  const habitos = await new Promise((res, rej) => {
    // trae los habitos del paciente
    const consulta = `SELECT  *
          FROM habitos h  
          WHERE h.dniPaciente = ? `;
    connection.query(consulta, [dniPaciente], function (err, results) {
  ...
  const atenciones = await new Promise((res, rej) => {
    // trae las atenciones previas del paciente
    const consulta = `SELECT  *
            FROM atenciones a  
            WHERE a.dniPaciente = ? `;
    connection.query(consulta, [dniPaciente], function (err, results) {
  ...
  const historial = await new Promise((res, rej) => {
    // trae el historial del paciente
  ...
  const evolucion = await new Promise((res, rej) => {
    // trae el historial de las evoluciones
  ...
  const plantillas = await new Promise((res, rej) => {
    // datos del paciente
...
app.post('/alergia', (req, res) => {
  ...
  connection.query(
    query,
    [dnipaciente, alergia, importancia, fechadesde, fechahasta],
    (err, results) => {
      //console.log(results);
      if (err) {
        console.error('error al insertar:', err);
...
app.post('/nuevaHistorial', (req, res) => {
  //agrega la historia
